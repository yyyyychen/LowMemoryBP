...
class RMSNormFunction(torch.autograd.Function):
    """
    Apply RMSNorm without affine transforms.
    """
    @staticmethod
    @torch.cuda.amp.custom_fwd
    def forward(ctx, x: torch.Tensor, normalized_shape: Sequence[int], eps: float):
        out, rstd = _C.rms_norm_fw(x, normalized_shape, eps)
        ctx.save_for_backward(out, rstd)
        return out
    ...
    @torch.autograd.function.once_differentiable
    def backward(ctx, out_grad):
        grad = _C.rms_norm_bw(out_grad, ctx.saved_tensors[0], ctx.saved_tensors[1])
        return grad, None, None


def rms_norm(input: torch.Tensor, normalized_shape: Sequence[int], eps: float) -> torch.Tensor:
    """
    Apply RMSNorm without affine transforms.
    """
    output = RMSNormFunction.apply(input, normalized_shape, eps)
    return output