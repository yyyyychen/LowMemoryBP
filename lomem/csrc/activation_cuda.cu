...
//     // double buffer
//     uint8_t flag [2]; 
//     uint8_t packed_flag [2];
//     vec_t input_vec [2];
//     vec_t output_vec [2];

//     input_vec[0] = *reinterpret_cast<vec_t*>(input_ptr + gid);
//     int gid_next = gid + num_threads * vec_size;
//     int gid_next2 = gid_next + num_threads * vec_size;

//     if (gid_next < N)
//         input_vec[1] = *reinterpret_cast<vec_t*>(input_ptr + gid_next);

//     packed_flag[0] = 0;
//     #pragma unroll
//     for (int k = 0; k < vec_size; ++k) {
//         resilu2_fw(input_vec[0].elem[k], output_vec[0].elem[k], flag[0]);
//         packed_flag[0] |= (flag[0] <<= (2 * (k & 3)));
//     }
//     packflagWarpReduce<vec_size * 2>(packed_flag[0]);

//     int load_buffer = 0;
//     int read_buffer {load_buffer ^ 1};
//     int & write_buffer {load_buffer};
//     #pragma unroll
//     for (int r = 0; r < inner_repeat; ++r) {
//         if (gid < N) {
//             if (gid_next2 < N)
//                 input_vec[load_buffer] = *reinterpret_cast<vec_t*>(input_ptr + gid_next2);

//             packed_flag[read_buffer] = 0;
//             #pragma unroll
//             for (int k = 0; k < vec_size; ++k) {
//                 resilu2_fw(input_vec[read_buffer].elem[k], output_vec[read_buffer].elem[k], flag[read_buffer]);
//                 packed_flag[read_buffer] |= (flag[read_buffer] <<= (2 * (k & 3)));
//             }
//             packflagWarpReduce<vec_size * 2>(packed_flag[read_buffer]);

//             *reinterpret_cast<vec_t*>(output_ptr + gid) = output_vec[write_buffer];
//             if (!(gid & 3)) *(flag_ptr + gid / 4) = packed_flag[write_buffer];
//         }
//         gid = gid_next;
//         gid_next = gid_next2;
//         gid_next2 += num_threads * vec_size;
//         load_buffer ^= 1;
//         read_buffer ^= 1;
//     }
// }
