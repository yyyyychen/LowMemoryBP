...
from timm.layers import DropPath, to_2tuple, trunc_normal_, resample_abs_pos_embed
from timm.models.registry import register_model

from utils import block_checkpoint
...
class vit_models(nn.Module):
    ...
    def __init__(self,
            ...
            init_scale=1e-4, block_ckpt=False, **kwargs
        ):
        super().__init__()
        ...
        self.block_ckpt = block_ckpt

    def partial_training(self, key_words=None):
    ...
    def resample_abs_pos_embed(self, img_size):
        num_patch_hw = img_size // self.patch_embed.patch_size[0]
        pos_need_grad = self.pos_embed.requires_grad
        self.pos_embed = nn.Parameter(resample_abs_pos_embed(
            self.pos_embed,
            (num_patch_hw, num_patch_hw),
            num_prefix_tokens=0
        ), requires_grad=pos_need_grad)
    ...
    def forward_features(self, x):
        ...
        x = block_checkpoint(self.blocks, x, self.block_ckpt)
    ...
    def load_finetuned_weight(self, path):
        # load finetuned weight
        state_dict = torch.load(path)['model']
        msg = self.load_state_dict(state_dict, strict=False)
        print("load fine-tuned weight:")
        print(msg)