...
def get_trainable_state_dict(model, keys=None):
    # when param is trainable or one of the keys is concluded in its name, save it
    def is_save(name, param):
    ...
    trainable_params = [n for n, p in model.named_parameters() if is_save(n, p)]
    model_state_dict = {n: p for n, p in model.state_dict().items() if n in trainable_params}
    model_state_dict.update({n: b for n, b in model.named_buffers()})
